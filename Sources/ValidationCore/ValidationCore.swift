...
public struct ValidationCore {
    ...
    public func validateExemption(encodedData: String, _ completionHandler: @escaping (ValidationResult)->()) {
        ...
        trustlistService.key(for: keyId, cwt: cwt, keyType: healthCert.type) { result in
            switch result {
            case .success(let key):
                guard cose.hasValidSignature(for: key) else {
    ...
    public func debug(encodedData: String, anonymizePersonalData: Bool, _ completionHandler: @escaping (ValidationResult) -> ()) {
        ...
        fileLogger.rollLogFile(withCompletion: {
            ...
            if anonymizePersonalData {
                cwt?.healthCert?.dateOfBirth = "***"
                cwt?.healthCert?.person.familyName = "***"
                cwt?.healthCert?.person.givenName = "***"
                cwt?.healthCert?.person.standardizedGivenName = "***"
                cwt?.healthCert?.person.standardizedFamilyName = "***"
            }
            DDLogInfo("CWT structure: \(cwt?.asJson() ?? "<n/a>")")
            let healthCert = cwt?.healthCert
            
            let trustlistDebugInfo = trustlistService.debugInformation(for: keyId, certType: healthCert?.type, cwt: cwt)
            ...
            trustlistService.key(for: keyId, keyType: healthCert?.type ?? .vaccination) { result in
                switch result {
                case .success(let key):
                    ...
                    completionHandler(ValidationResult(isValid: errors.isEmpty, metaInformation: MetaInfo(from: cwt, signatureCertInfo: trustlistDebugInfo.signatureCertInfo, trustlistInfo: trustlistDebugInfo.trustlistInfo, errors: errors), greenpass: healthCert, error: errors.first))
                    return
                case .failure(let error):
                    errors.append(error)
                    DDLogInfo("Greenpass seems invalid")
                    completionHandler(ValidationResult(isValid: false, metaInformation: MetaInfo(from: cwt, signatureCertInfo: trustlistDebugInfo.signatureCertInfo, trustlistInfo: trustlistDebugInfo.trustlistInfo, errors: errors), greenpass: healthCert, error: errors.first))
                }
                DDLog.flushLog()
            }
       })
